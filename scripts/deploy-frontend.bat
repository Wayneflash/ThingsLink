...
echo   前端路径: !REMOTE_FRONTEND_PATH!
echo.
echo ⚠️  请确认以上配置是否正确！
echo.
set /p CONFIRM="确认部署前端？(Y/N): "
if /i not "!CONFIRM!"=="Y" (
    echo 部署已取消
    pause
    exit /b 0
...
if not exist "!LOCAL_FRONTEND_DIST!" (
    echo [ERROR] 前端dist目录不存在: !LOCAL_FRONTEND_DIST!
    echo.
    echo 请先编译前端: 双击 scripts\build-frontend.bat
    pause
    exit /b 1
)
...
echo [OK] 找到dist目录: !LOCAL_FRONTEND_DIST!
echo.
...
REM 检查并创建远程目录
echo [2/4] 检查远程目录...
ssh -p !REMOTE_PORT! !REMOTE_USER!@!REMOTE_HOST! "mkdir -p !REMOTE_FRONTEND_PATH!" >nul 2>&1
if errorlevel 1 (
...
cd ..
echo [OK] 打包完成: frontend-dist.tar.gz
echo.
...
if errorlevel 1 (
    ...
    echo   2. 远程路径不存在（需要手动创建：!REMOTE_FRONTEND_PATH!）
...
REM 远程解压
echo [5/5] 解压前端文件...
ssh -p !REMOTE_PORT! !REMOTE_USER!@!REMOTE_HOST! "cd !REMOTE_FRONTEND_PATH! && tar -xzf frontend-dist.tar.gz && rm -f frontend-dist.tar.gz"
if errorlevel 1 (
    echo [ERROR] 解压失败
    echo.
    echo 请手动执行：
    ...
    echo   rm -f frontend-dist.tar.gz
    echo.
    del frontend-dist.tar.gz 2>nul
    pause
    exit /b 1
...
echo.
echo 注意: 前端是静态文件，通常不需要重启服务
echo       如果使用Nginx，可能需要: systemctl reload nginx
echo.
pause
endlocal
