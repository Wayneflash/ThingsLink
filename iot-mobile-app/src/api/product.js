...
/**
 * 获取产品属性列表
 * @param {Object|number} productIdOrParams - 产品ID（数字）或参数对象（包含productId）
 * @returns {Promise} 返回属性列表
 */
export const getProductAttributes = (productIdOrParams) => {
  // 兼容两种调用方式：直接传productId数字，或传参数对象
  const productId = typeof productIdOrParams === 'object' 
